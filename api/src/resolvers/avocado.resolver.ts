...
export function findAll(parent: unknown, arg: {skip?: number, take?: number, where: Prisma.AvocadoWhereInput}, context: ResolverContext): Promise<Avocado[]> {
...
export function createAvo(
  ...
  return context.orm.avocado.create({
    data: {
      ...
      price,
      image,
      sku: new Date().getTime().toString(), // Esta es una solucion rapida. El sku se deberia pedir en el input de graphql.
      attributes: {
        create: {
          ...attributes,
        }
